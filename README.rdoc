...
_This is not an official version. Please check the Deviations section below
for the list of changes._
...
Here's the list of what's added in this fork:

1. Added the ability to specify custom tags in the :url and :path options of the
   has_attached_file definition, like that (note that):

    has_attached_file :image, :url => "/assets/photos/:project_id/:style.jpg",
      :tags => { :project_id => lambda { |attachment, style| attachment.instance.project.id } }

2. Added easy access to attachment geometry:

    has_attached_file :image, :styles => { :medium => ..., ... }

    <%= model.image.width(:medium) %>
    <%= model.image.height(:medium) %>
